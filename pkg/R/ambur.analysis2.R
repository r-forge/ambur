...
write.table(WorkTable1a, file = "debugging3WorkTable1a.csv", sep = ",", row.names = TRUE)
...
write.table(FinalTable, file = "results_stats_plotting.csv", sep = ",", row.names = TRUE)
...
write.table(FinalTable2, file = "GIS_stats_table_long.csv", sep = ",", row.names = FALSE, quote = FALSE)
...
write.table(DebugTable, file = "debugging.csv", quote = FALSE, sep = ",", row.names = FALSE)
...
#Plot of histograms for EPR,LRR,WLR  with no "breaks"

pdf("PDF/histograms_2.pdf",width = 6.5, height = 6.5, bg="white", paper= "letter" )
par(mfrow=(c(3,3)))
par(pty= "m")

hist(EPR, main= "", xlab= "", ylab="")
hist(LRR.slope, main= "All Transects", xlab= "", ylab="")
hist(WLR.slope, main= "", xlab="", ylab="")

hist(ifelse(EPR <0, EPR, 0), main= "", xlab= "", ylab="Frequency")
hist(ifelse(LRR.slope <0, LRR.slope, 0), main= "Erosion Transects", xlab= "", ylab="")
hist(ifelse(WLR.slope <0, WLR.slope, 0), main= "", xlab= "", ylab="")


hist(ifelse(EPR >0, EPR, 0), main= "", xlab= "EPR", ylab="")
hist(ifelse(LRR.slope >0, LRR.slope, 0), main= "Accretion Transects", xlab= "LRR.slope", ylab="")
hist(ifelse(WLR.slope >0, WLR.slope, 0), main= "", xlab= "WLR.slope", ylab="")
...
myLegendText <- c("Erosion", "Accretion", "Flagged Transect")
...
#map - basic shoreline plot of Net Change trend
pdf("PDF/map_net_change_2.pdf", width = 10, height = 10, bg="white")

plot(Transect.Outer.Xcoord, Transect.Outer.Ycoord, type="l", lwd= 0, col= "white" , asp=1, xlab=paste("X ","(",Map.Units,")",sep=""),ylab=paste("Y ","(",Map.Units,")",sep=""))
...
#Plot 2 Average Rates of Consecutive Eras
...
mtext("SHORELINE CHANGE SUMMARY", side=3, line= 3, adj= 0.5, cex= 0.5)

mtext("AMBUR v.1.0 (20100526)", side=3, line= 2, adj= 0.5, cex= 0.5)


mtext("Oldest Shoreline Date:", side=3, line= 1, adj= 0, cex= 0.5)
mtext(as.character(min(DATE)), side=3, line= 1, adj= 1, cex= 0.5)
 
 
mtext("Youngest Shoreline Date:", side=3, line= 0, adj= 0, cex= 0.5)
...
#Plot 6  Range of change
plot(Transect, Range.Distance, type="l", lwd= 0, col= "white" , las= 1, cex.axis= 0.7, cex.lab= 0.7,xlab=expression(paste("Transect")),ylab=paste("Range of Change ","(",Map.Units,")",sep=""))
...
#Cumulative shoreline evolution by date

pdf("PDF/graph_cumulative_change_2.pdf", width = 9, height = 6, bg="white")
...
plot(x, y, cex = z, xlab = "Transect", ylab = "", col= Changes.Colors, las= 1, cex.axis= 0.7, cex.lab= 0.7, bty= "l", yaxt= "n")

points(Changes.Transects[Changes.Distances == 0], Changes.DatesSecs[Changes.Distances == 0], col= "green", type= "p", pch= 39)

axis(2, at= Changes.DatesSecs, labels= as.character(format(as.POSIXct(Changes.DatesSecs , origin="1970-01-01"),"%m/%d/%Y %I:%M:%S %p")), cex.axis= 0.5, las= 1)

dev.off()

#Presence of dates at transects

pdf("PDF/graph_dates_present.pdf", width = 9, height = 6, bg="white")
...
plot(x, y, xlab = "Transect", ylab = "", col= "black", las= 1, cex.axis= 0.7, cex.lab= 0.7, bty= "l", yaxt= "n", pch=39)
...
pdf("PDF/graph_dates_present_2.pdf")
...
#plot shoreline change grids

Window.Labels  <- as.character((tt3)[-1] + as.Date("1970-01-01"))

Transect.Labels  <- as.numeric(unq.transects %in% pretty(unq.transects) * unq.transects)
Transect.Labels <- ifelse(Transect.Labels==0,"",Transect.Labels*1)

pdf("PDF/graph_eras_grid1.pdf", bg="white")

print(levelplot(chng.eras, pretty=TRUE,na.rm=TRUE,colorkey = list(space="bottom",tick.number=1,height=0.5,width=1,cex=2, at=c(-0.002,-0.001,0,0.001,0.002),labels=c("","Erosion","","Accretion","")),at= c(min(chng.eras,na.rm=TRUE),0,max(chng.eras,na.rm=TRUE)),col.regions=c("red","red","blue","blue"),xlab="Transect",ylab="Era",scale = list(x = list(labels=Transect.Labels),y = list(labels=Window.Labels,cex=0.5)),page = function(n) grid.text(paste(""), x = 0.5275, y = 0.00001, default.units = "npc", just = c("center", "bottom")),aspect="iso"))
...
print(levelplot(chng.eras, pretty=TRUE,na.rm=TRUE,colorkey = list(space="bottom"),col.regions=rainbow,xlab="Transect",ylab="Era",scale = list(x = list(labels=Transect.Labels),y = list(labels=Window.Labels,cex=0.5))))
...
print(levelplot(rates.eras, pretty=TRUE,na.rm=TRUE,colorkey = list(space="bottom"),col.regions=rainbow,xlab="Transect",ylab="Era",scale = list(x = list(labels=Transect.Labels),y = list(labels=Window.Labels,cex=0.5))))
