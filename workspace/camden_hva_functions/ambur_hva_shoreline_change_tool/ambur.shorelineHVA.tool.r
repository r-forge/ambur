...
hva_columns <- (length(colnames(merge.tab2)) - (length(rename2)-1)):(length(colnames(merge.tab2)))

merge.tab2[,hva_columns][merge.tab2[,hva_columns]==0] <- NA

hva.product <- apply(merge.tab2[,hva_columns],1,prod,na.rm=TRUE)

test.variables <-  ifelse(is.na(merge.tab2[,hva_columns]) == TRUE, 0, 1)      ###get number of variables present
...
row.names(merge.tab2) <- as.numeric(row.names(attrtable2))

lns.output <- SpatialLinesDataFrame(shapedata2,merge.tab2)

    projectionString <- proj4string(shapedata2) # contains projection info

  proj4string(lns.output) <- projectionString

writeOGR(lns.output, ".", "ambur_hva_calculations", driver="ESRI Shapefile")

pts.prep <- SpatialPoints(cbind(merge.tab2$Max_DateX,merge.tab2$Max_DateY))
pts.output <- SpatialPointsDataFrame(pts.prep,merge.tab2)
proj4string(pts.output) <- projectionString
writeOGR(pts.output, ".", "ambur_hva_calculations_pts", driver="ESRI Shapefile")
