...
attrtable <- data.frame(shapedata)
 ...
 tkmessageBox(message = "Please select the SLOSH shapefile...")
filters <- matrix(c("Shapefile", ".shp"), 1, 2, byrow = TRUE)
...
hva.calc <- sqrt((tet3$hva_cat.x * tet3$hva_cat.y)/2)

tet3$hva_calc <- hva.calc
...
writeOGR(outputdata, ".", "inundation_results", driver="ESRI Shapefile")
