...
tkmessageBox(message = "Please select the shoreline shapefile with sea level trends...")
...
ifelse(colnames(tet3) == "StartX", tet3$Start_X <- tet3$StartX,   tet3$StartX <- tet3$Start_X)
ifelse(colnames(tet3) == "StartY", tet3$Start_Y <- tet3$StartY,   tet3$StartY <- tet3$Start_Y)
...
SeaLevel_values <- shapetablecull[,"SeaLevelRt"]
hva_cat <- numeric(length(SeaLevel_values)) 
######HVA index setup

hva_cat[shapetablecull[,"SeaLevelRt"] <= 0] <- 1
hva_cat[shapetablecull[,"SeaLevelRt"] > 0] <- 2
hva_cat[shapetablecull[,"SeaLevelRt"] > 2] <- 3
hva_cat[shapetablecull[,"SeaLevelRt"] > 4] <- 4
hva_cat[shapetablecull[,"SeaLevelRt"] > 6] <- 5
...
sealevel_hva_table <- cbind(shapetablecull$Transect, hva_cat)

colnames(sealevel_hva_table) <- c("Transect","hva_cat")

write.table(sealevel_hva_table, file = "ambur_hva_sealevel_data.csv", sep = ",", row.names = FALSE)
...
writeOGR(pts.output, ".", "ambur_hva_sealevel_pts", driver="ESRI Shapefile")
