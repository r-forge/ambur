...
shapedata@data <- shapedata@data[,-(1:3)]  #get rid of transID and shoreID
...
outputdata <- SpatialLinesDataFrame(out,tet3)
 ...
 writeOGR(outputdata, ".", "ambur_inundation_lines_all", driver="ESRI Shapefile") 
...
testblah3 <- testblah2[,3]
 ...
 ######HVA index setup
hva_cat <- shapetablecull[,"hva_scaled"]

shapetablecull$hva_cat <- hva_cat          


inun_hva_table <- cbind(shapetablecull$Transect, hva_cat)

colnames(inun_hva_table) <- c("Transect","hva_cat")

write.table(inun_hva_table, file = "ambur_hva_inun_data.csv", sep = ",", row.names = FALSE)
...
writeOGR(lns.output, ".", "ambur_hva_inun_lns", driver="ESRI Shapefile")
