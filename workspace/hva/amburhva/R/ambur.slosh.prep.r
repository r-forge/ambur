...
outputdata <- SpatialLinesDataFrame(out,tet3)
 ...
 writeOGR(outputdata, ".", "ambur_slosh_lines_all", driver="ESRI Shapefile") 
...
hva_cat[shapetablecull[,"c5_mean"] >= 1] <- 1
hva_cat[shapetablecull[,"c4_mean"] > 1] <- 2
hva_cat[shapetablecull[,"c3_mean"] > 1] <- 3
hva_cat[shapetablecull[,"c2_mean"] > 1] <- 4
hva_cat[shapetablecull[,"c1_mean"] > 1] <- 5

shapetablecull$hva_cat <- hva_cat          


slosh_hva_table <- cbind(shapetablecull$Transect, hva_cat)

colnames(slosh_hva_table) <- c("Transect","hva_cat")

write.table(slosh_hva_table, file = "ambur_hva_slosh_data.csv", sep = ",", row.names = FALSE)
...
writeOGR(lns.output, ".", "ambur_hva_slosh_lns", driver="ESRI Shapefile")
